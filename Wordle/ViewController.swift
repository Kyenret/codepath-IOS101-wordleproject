    ...
    // START YOUR CODE HERE
      keyboardController.didSelectString = { [self] selectedString in
          if selectedString == DELETE_KEY {
              self.boardController.deleteLastCharacter() //call the `deleteLastCharacter` method of `boardController`. (question: why is it making me add self?)
          }else{
              self.boardController.enter(selectedString) //Else, it should use the `enter` method of `boardController` and pass in the selected string as the argument
          }
